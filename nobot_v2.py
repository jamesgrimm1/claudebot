...
║  Base % of bankroll (REVERSED — higher price = more):    ║
║  $0.45–$0.49 → 0.210% × volume_mult                     ║
║  $0.50–$0.52 → 0.318% × volume_mult                     ║
║  $0.53–$0.55 → 0.310% × volume_mult                     ║
...
TELEGRAM_CHANNEL_ID  = ""  # disabled — no public channel for V1/V2
...
STAKE_BRACKETS = [
    {"min_no": 45, "max_no": 49, "pct": 0.210},   # lowest prob  → smallest base
    {"min_no": 50, "max_no": 52, "pct": 0.318},
    {"min_no": 53, "max_no": 55, "pct": 0.310},
    {"min_no": 56, "max_no": 58, "pct": 0.410},
...
def get_stake(no_price, bankroll, volume=0):
    """
    V2 sizing: reversed brackets (higher NO = bigger base %)
    multiplied by volume tier. Compounds with bankroll.
    """
    # Get base % for this NO price bracket
    base_pct = None
    for bracket in STAKE_BRACKETS:
        if bracket["min_no"] <= no_price <= bracket["max_no"]:
            base_pct = bracket["pct"]
            break
    if base_pct is None:
        return 0.0
    ...
    mult = 1.0
    for tier in VOLUME_MULTIPLIERS:
        if volume >= tier["min_vol"]:
            mult = tier["mult"]
            break

    stake = round(bankroll * base_pct / 100 * mult, 2)
...
def place_trade(market, state):
    ...
    stake    = get_stake(no_price, state["bankroll"], volume=market.get("volume", 0))
...
def print_portfolio(state):
    ...
    print("\n" + "═" * 65)
    print("  NOBOT V2  ·  'Nothing Ever Happens'  ·  Reversed + Volume Sizing")
    print("═" * 65)
...
def single_scan():
    now = datetime.now(timezone.utc)
    print("\n╔══════════════════════════════════════════════════════════╗")
    print("║  NOBOT V2  ·  'Nothing Ever Happens'  ·  Reversed + Vol      ║")

# ═══════════════════════════════════════════════════════
# NearCertain BETA — Data Collection Mode
# Micro stakes ($0.50), all categories, all time frames
# No Telegram, no daily loss limit, no position cap
# Purpose: accumulate category-level WR data
# ═══════════════════════════════════════════════════════
...
TELEGRAM_CHANNEL_ID  = ""  # Beta: Telegram disabled
TELEGRAM_PERSONAL_ID = ""  # Beta: Telegram disabled

SCREENER_MODEL     = "claude-haiku-4-5-20251001"
PAPER_TRADING      = True
STARTING_BANKROLL  = 1000.00
LOG_FILE           = "nearcertain_beta_log.json"
...
MAX_HOLD_DAYS  = 180.0  # Beta: no upper time limit
MIN_HOLD_HOURS = 0      # Beta: no lower time limit
...
STAKE_TIERS = [
    {"max_no": 99, "pct": 0.05},   # Beta: flat micro-stake ~$0.50 per trade for data collection
]
BETA_FIXED_STAKE = 0.50            # Hard floor — never stake less than this

MIN_VOLUME         = 2000    # default min volume
MIN_VOLUME_WEATHER = 500     # weather markets are small — allow lower volume
DAILY_LOSS_LIMIT  = 999999.00  # Beta: no daily loss limit — pure data collection
NEWS_LOOKBACK_HOURS = 72
MAX_OPEN_POSITIONS  = 9999  # Beta: no cap — collect as much data as possible
...
BLOCKED_CATEGORIES = set()  # Beta: no blocked categories — collect data on everything
...
def telegram_scan_summary(new_trades, resolved_trades, state):
    """Send one summary message per scan instead of one per trade."""
    ...
    lines = ["🔵 <b>NEARCERTAIN — Scan Summary</b>\n"]

    if resolved_trades:
        lines.append("📋 <b>Resolved this scan:</b>")
        for t in resolved_trades:
            emoji = "✅" if t.get("won") else "❌"
            pnl   = t.get("realized_pnl", 0)
            lines.append(f"{emoji} ${pnl:+.2f} — {t['market'][:50]}")
        lines.append("")

    if new_trades:
        lines.append("🆕 <b>New positions:</b>")
        for t in new_trades:
            pot = t.get("potential_profit", 0)
            lines.append(
                f"NO@{t['entry_no_price']}¢ | ${t['stake']:.2f} → +${pot:.2f} if win | "
                f"{t['market'][:45]}"
            )
        lines.append("")

    if not new_trades and not resolved_trades:
        lines.append("No new trades or resolutions this scan.")
        lines.append("")

    lines.append(
        f"💰 Bankroll: ${state['bankroll']:.2f} ({roi:+.1f}%)\n"
        f"📊 {len(open_t)} open | {len(won)}W/{len(closed)-len(won)}L ({wr:.0f}% WR)"
    )
    telegram_send("\n".join(lines))
...
def fetch_markets():
    ...
    try:
        ...
        for m in raw_markets:
            ...
            # Category needed before volume check so weather gets lower threshold
            cat = get_category(m["question"])

            # Volume filter — weather markets are small, allow lower threshold
            vol_min = MIN_VOLUME_WEATHER if cat == "weather" else MIN_VOLUME
            if float(m.get("volume", 0)) < vol_min:
                skipped += 1
                continue
...
def place_trade(market, state):
    ...
    trade = {
        ...
        "stake":            stake,
        "potential_profit": round(stake * 100 / no_price - stake, 2),
        "potential_payout": round(stake * 100 / no_price, 2),
...
def single_scan():
    ...
    trades_before = {t["id"] for t in state["trades"] if t["status"] == "open"}
    resolve_open_trades(state)
    resolved_this_scan = [t for t in state["trades"] if t["status"] == "closed" and t["id"] in trades_before]
    ...
    if not markets:
        ...
        telegram_scan_summary([], resolved_this_scan, state)
        return
    ...
    ids_before_placing = {t["id"] for t in state["trades"]}
    placed = 0
    for market in candidates:
        place_trade(market, state)
        placed += 1

    new_trades = [t for t in state["trades"] if t["id"] not in ids_before_placing]
    ...
    # ── Step 5: Save + Telegram summary ──────────────────
    log("── Step 5: Save ─────────────────────────────────────────")
    save_state(state)
    print_portfolio(state)
    telegram_scan_summary(new_trades, resolved_this_scan, state)

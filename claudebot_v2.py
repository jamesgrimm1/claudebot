"""
╔══════════════════════════════════════════════════════════╗
║      CLAUDEBOT V2 — Aggressive Companion to ClaudeBot    ║
║                                                          ║
║  Same Opus brain, same research, same news monitor.      ║
║  Tuned for HIGHER VOLUME — not lower confidence.         ║
║                                                          ║
║  Differences from V1:                                    ║
║  • T1 only — no T2 medium-term tier                      ║
║  • min_edge 10% (was 15%)                                ║
║  • max_positions 12 (was 6)                              ║
║  • min_hold_hours 0.5 (was 2) — allows fast scalps       ║
║  • No per-trade stake cap (Kelly runs free)              ║
║  • All categories allowed (sports unblocked)             ║
║  • Per-category cap 3 (was 1) — more concurrent in cat   ║
║  • Screener returns 15 candidates (was 10)               ║
║  • Scan interval 60min (was 180)                         ║
║                                                          ║
║  Min confidence stays at 75% — research is the edge,     ║
║  not threshold. Opus reasoning is unchanged.             ║
║                                                          ║
║  Runs in parallel with V1 — separate log, separate       ║
║  bankroll, may take same trades independently.           ║
║                                                          ║
║  SETUP:  pip install anthropic requests ddgs feedparser  ║
║  RUN:    python claudebot_v2.py --single-scan            ║
╚══════════════════════════════════════════════════════════╝
"""
...
LOG_FILE           = "claudebot_v2_log.json"
...
REASSESS_INTERVAL_DAYS = 3   # T2/T3 reassess (V2 has no T2/T3 — kept for compatibility)
MIN_DAYS_REMAINING     = 5   # skip reassessment if trade closes within N days

BLOCKED_CATEGORIES = set()  # V2: all categories allowed

TIERS = {
    1: {
        "name":           "Short-term",
        "label":          "T1",
        "min_hold_hours": 0.5,        # V2: was 2, allow rapid scalp entries
        "max_hold_days":  7,
        "min_confidence": 75,         # SAME as V1 — research is the edge, not threshold
        "min_edge_pct":   10,         # V2: was 15, accept slightly thinner edges
        "max_positions":  12,         # V2: was 6, more concurrent positions
        "fixed_pct":      None,
        "kelly": [
            {"min_conf": 90, "fraction": 1.0, "max_pct": 100.0},  # V2: no per-trade cap
            {"min_conf": 75, "fraction": 0.5, "max_pct": 100.0},  # V2: no per-trade cap
        ],
        "short_disc_1d":  0.65,
        "short_disc_2d":  0.80,
        "screener_top_n": 15,         # V2: was 10, more candidates per scan
        "screener_max_per_cat": 3,    # V2: was 2, slightly more diversity
    },
    # T2 removed — V2 is T1-only
}

MAX_PER_CATEGORY = 3   # V2: was 1, allow more concurrent in same category
...
def single_scan():
    ...
    print(f"║  {now.strftime('%Y-%m-%d %H:%M UTC')}  |  T1 every scan, no T2/T3       ║")
    print("╚══════════════════════════════════════════════════════════╝\n")
    ...
    # ── Step 1b: Reassessment brain — V2 has no T2/T3 so this is a no-op
    # but the function call is harmless (returns state unchanged when no T2 trades)
    log("── Reassessment: V2 is T1-only, skipping ──────────────")

    # ── Tier 1 — always ──────────────────────────────────
    log("── Tier 1: Short-term (V2: 0.5h-7d, edge 10%, max 12 pos) ──")
    state = run_tier(1, state, priority_markets=priority_markets)

    # ── No Tier 2 in V2 ──────────────────────────────────
    # (T2 removed — V2 focuses entirely on T1 turnover)
...
def run_loop():
    print("\n╔══════════════════════════════════════════════════════════╗")
    print("║  CLAUDEBOT V2  ·  T1-only aggressive + News Monitor      ║")
    print(f"║  Interval: {SCAN_INTERVAL_MINS}min  |  T1 every scan, no T2/T3        ║")
    print("╚══════════════════════════════════════════════════════════╝\n")

...
║  Opus analyzes what's working → updates alpha_prime_config.json ║
...
def analyse_trades(trades, label, lookback_days=LOOKBACK_DAYS):
    ...
    for band, v in sorted(bands.items(), key=lambda x: int(x[0].split("-")[0])):
        ...
        lines.append(f"  YES {band}¢  {n:>3} trades | {v['w']}W/{v['l']}L | {wr_b:.0f}% WR | ${v['pnl']:+.2f}")
...
def parse_and_save_config(raw_text):
    ...
    if not match:
        log("⚠️  Could not find JSON in Opus response — keeping existing config")
        return False

    try:
        config = json.loads(match.group(0))
    except json.JSONDecodeError as e:
        log(f"⚠️  JSON parse error: {e} — keeping existing config")
        return False
    ...
    for key in required:
        if key not in config:
            log(f"⚠️  Missing required key '{key}' — keeping existing config")
            return False

    # Sanity checks
    if config["yes_price_min"] < 65 or config["yes_price_max"] > 99:
        log("⚠️  YES price range out of bounds — keeping existing config")
        return False

    with open(AP_CONFIG, "w") as f:
        json.dump(config, f, indent=2)

    log(f"✅ AlphaPrime config updated → {AP_CONFIG}")
    log(f"   Blocked: {config.get('blocked_categories', [])}")
    log(f"   YES range: {config.get('yes_price_min')}-{config.get('yes_price_max')}¢")
...
def main():
    now = datetime.now(timezone.utc)
    print("\n╔══════════════════════════════════════════════════════════╗")
    print("║  SELF AUDIT  ·  NearCertain → AlphaPrime Auto-Updater    ║")
    ...
    if success:
        ...
        log("\n✅ Self-audit complete. AlphaPrime will use updated config on next scan.")
    else:
        log("\n⚠️  Audit completed but config not updated — check logs above")
